...
def gradio_ask(user_message, chatbot, chat_state):
    ...
    return (
        gr.update(
            interactive=False,
            placeholder="Currently only single round conversations are supported.",
        ),
        chatbot,
        chat_state,
    )
...
def gradio_answer(chatbot, chat_state, num_beams, temperature, top_p):
    ...
    prompt = [cfg.config.model.prompt_template.format(chat_state[1].strip())]
    with torch.cuda.amp.autocast(dtype=torch.float16):
        llm_message = model.generate(samples, cfg.config.generate, prompts=prompt)
...
description = (
    """<h3>This is the demo of SALMONN. Upload your audio and start chatting!</h3>"""
)
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column():
            ...
            text_input = gr.Textbox(
                label="User",
                placeholder="Please upload your speech first",
                interactive=False,
            )
    ...
    with gr.Row():
        examples = gr.Examples(
            examples=[
                ...
                [
                    "resource/audio_demo/gunshots.wav",
                    "Recognize what the speaker says and describe the background audio at the same time.",
                ],
                [
                    "resource/audio_demo/gunshots.wav",
    ...
    upload_button.click(
        upload_speech,
        [speech, text_input, chat_state],
        [speech, text_input, upload_button, chat_state],
    )

    text_input.submit(
        gradio_ask, [text_input, chatbot, chat_state], [text_input, chatbot, chat_state]
    ).then(gradio_answer, [chatbot, chat_state], [chatbot, chat_state])
    clear.click(
        gradio_reset,
        [chat_state],
        [chatbot, speech, text_input, upload_button, chat_state],
        queue=False,
    )
