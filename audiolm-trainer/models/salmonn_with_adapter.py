...
class ADAPTERSALMONN(nn.Module):
    @classmethod
    ...
    def __init__(
        ...
        self.llama_tokenizer = AutoTokenizer.from_pretrained(
            llama_path, use_fast=False, token=token
        )
        ...
        if self.beats_path:
            ...
            beats_ckpt = torch.load(
                self.beats_path, map_location="cpu", weights_only=True
            )
        ...
        if self.use_speech_Qformer:
            if self.beats_path:
                self.speech_Qformer, self.speech_query_tokens = (
                    self.init_speech_Qformer(
                        num_query_token=num_speech_query_token,
                        speech_width=self.speech_encoder.config.d_model
                        + self.beats.cfg.encoder_embed_dim,
                    )
                )
            else:
                self.speech_Qformer, self.speech_query_tokens = (
            ...
            if speech_llama_proj_model:
                logging.info(
                    f"Loading speech LLAMA proj from {speech_llama_proj_model}"
                )
                speech_llama_proj_weight = torch.load(
                    speech_llama_proj_model, map_location="cpu"
                )
        ...
        if prompt_path:
            try:
                raw_prompts = json.load(open(prompt_path))
            except:
                print("Failed to load prompt! Try to use utf-8 encoding.")
                raw_prompts = json.load(open(prompt_path, encoding="utf-8"))
            for task in raw_prompts.keys():
                filted_prompts = [
                    raw_prompt
                    for raw_prompt in raw_prompts[task]
                    if "<SpeechHere>" in raw_prompt
                ]
                self.prompt_dict[task] = [
    ...
    def _encode_auditory_feature(self, speech_embeds, audio_embeds=None):
        """
        speech_embeds와 audio_embeds를 입력받아 합친 후 Qformer로 쿼리

        Args:
            speech_embeds: torch.Tensor, [B, T, C]
            audio_embeds: torch.Tensor, [B, T, C]

        Returns:
            speech_embeds: torch.Tensor, [B, T, C]
            speech_atts: torch.Tensor, [B, T]
        """
        with self.maybe_autocast():
            if self.use_speech_Qformer:
                # Concatenate speech and audio embeddings
                ...
                if audio_embeds is not None:
                    ...
                    if audio_embeds.size(1) < speech_embeds.size(1):
                        audio_embeds = F.pad(
                            audio_embeds,
                            (0, 0, 0, speech_embeds.size(1) - audio_embeds.size(1)),
                        )
                    elif audio_embeds.size(1) > speech_embeds.size(1):
                        speech_embeds = F.pad(
                            speech_embeds,
                            (0, 0, 0, audio_embeds.size(1) - speech_embeds.size(1)),
                        )
                    speech_embeds = torch.cat((speech_embeds, audio_embeds), dim=-1)
                speech_atts = torch.ones(
                    speech_embeds.size()[:-1], dtype=torch.long
                ).to(speech_embeds.device)

                # Convert speech_embeds into window-sized embeds
                if self.window_level_Qformer:
                    ...
                    speech_embeds_overlap = speech_embeds_overlap.view(
                        B, -1, kernel[1], L
                    )
    ...
    def encode_speech(self, spectrogram, raw_wav=None, audio_padding_mask=None):
        """
        Whisper 인코더(self.speech_encoder)와 Beats 인코더(self.beats)로 음성/오디오 임베딩을 생성

        Args:
            spectrogram: torch.Tensor, [B, T, C]
            raw_wav: torch.Tensor, [B, T]
    ...
    def prompt_wrap(self, embeds, atts, prompt, multi_prompt=False):
        """
        주어진 음성 임베딩에 프롬프트를 추가해 확장된 입력 임베딩과 어텐션 마스크를 생성. 음성 및 텍스트 데이터를 함께 처리하는데 사용.

        Args:
            embeds: torch.Tensor, [B, T, C] - 입력 임베딩 텐서
            atts: torch.Tensor, [B, T] - 입력 어텐션 마스크 텐서
            prompt: str - 사용할 프롬프트 문자열
            multi_prompt: bool - 여러 프롬프트를 사용할지 여부 (기본값: False)

        Returns:
            wrapped_embeds: torch.Tensor, [B, T, C] - 프롬프트로 래핑된 임베딩 텐서
            wrapped_atts: torch.Tensor, [B, T] - 프롬프트로 래핑된 어텐션 마스크 텐서
        """
        if prompt:
            if multi_prompt:
                p_before = []
                ...
                wrapped_embeds = torch.cat(
                    [p_before_embeds, embeds, p_after_embeds], dim=1
                )
    ...
    def forward(self, samples, verbose=False):
        ...
        if self.prompt_dict:
            if self.multi_prompt:
                prompt = [
                    random.choice(self.prompt_dict[task]) for task in samples["task"]
                ]
                if "Q" in samples:
                    prompt = [
                        p.format(q) if "{}" in p else p
                        for p, q in zip(prompt, samples["Q"])
                    ]
            else:
        ...
        empty_targets = (
            torch.ones(
                [speech_atts.shape[0], speech_atts.shape[1] + 1], dtype=torch.long
            )
            .to(spectrogram.device)
            .fill_(-100)
        )

        # generate targets and bos tokens
        ...
        if verbose:
            ...
            results = (
                outputs.logits[:, empty_targets.size(1) - 1 : -1, :]
                .contiguous()
                .view(-1, nvocab)
                .argmax(dim=-1)
            )
    ...
    def generate(self, samples, generate_cfg, prompts=None):
        ...
        stop_words_ids = [
            torch.tensor([2]).to(speech_embeds.device)
        ]  # TODO: fix this heuristics
    ...
    def from_config(cls, config):
        ...
        if ckpt_path:
            logging.info(f"Load SALMONN ckpt from: {ckpt_path}")
